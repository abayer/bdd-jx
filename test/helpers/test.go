...
func (t *TestOptions) GetApproverGitProvider() (gits.GitProvider, error) {
	...
	if userAuth == nil {
		userAuth = config.GetOrCreateUserAuth(authServer.URL, PRApproverUsername)
		userAuth.ApiToken = PRApproverToken
		userAuth.Password = PRApproverToken
		err = authConfigService.SaveConfig()
		if err != nil {
			return nil, err
		}
	}
...
// ApprovePRFromLogOutput takes the default provider, the approver user's provider, git info, and the output from a command that
// created a PR, and adds the approver user as a collaborator, accepts the invitation, and approves the PR.
func (t *TestOptions) ApprovePRFromLogOutput(provider gits.GitProvider, approverProvider gits.GitProvider, gitInfo *gits.GitRepository, output string) {
